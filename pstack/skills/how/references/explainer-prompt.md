...
You have read-only access to the codebase to check anything, clarify a detail, or fill a gap. Use Read, Grep, and Glob as needed. Read-only: do not edit, write, or commit anything. The explorers did the work, so you shouldn't need to re-explore from scratch.
