...
`/how`, `/why`, and `/architect` are user-only skills. To run one, read its `SKILL.md` and follow it, resolved as a sibling of this skill (`<base directory>/../<name>/SKILL.md`), then `~/.claude/skills/<name>/SKILL.md`, then `.claude/skills/<name>/SKILL.md`, then `find ~/.claude/plugins -path '*/pstack/skills/<name>/SKILL.md'`.

1. Spawn a subagent with the `Agent` tool and `subagent_type: "comment-sicko"`. Pass the scope and the absolute paths of the **how** and **why** skills' `SKILL.md`. Do not restate its rules.
