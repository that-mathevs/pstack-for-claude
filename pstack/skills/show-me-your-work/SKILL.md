...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript at `~/.claude/projects/<slug>/$CLAUDE_CODE_SESSION_ID.jsonl`, and its subagents' transcripts under `~/.claude/projects/<slug>/$CLAUDE_CODE_SESSION_ID/subagents/`. `<slug>` is the absolute working directory with every character that is not a letter or digit replaced by `-`. Don't glob across `~/.claude/projects/*/`. That reads unrelated private chats. The JSONL is an internal format, so skip lines that fail to parse. Walk the log against what actually happened:
...
Before handing back, spawn a subagent on a different model tier from the one that did the work (`model: "fable"`, `"opus"`, or `"sonnet"`). Self-review is not a substitute. Claude tiers differ less than models from different vendors, so a different tier is the weaker form of this check. Pass the reviewer the log path and the transcript paths. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
