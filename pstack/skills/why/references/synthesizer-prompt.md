...
5. **Verify citations by spot-checking.** You can read the codebase and call MCP tools to verify citations. Read-only: do not edit, write, or commit anything, and do not modify external state. If you're uncertain a cited item exists or says what's claimed, check it. Don't propagate errors.
