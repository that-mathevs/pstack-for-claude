...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles comments from PR review bots or other review automation: Cursor Bugbot, Claude Code's `/code-review --comment`, Copilot, CodeRabbit, and agentic security reviews. The rules apply to any of them. The goal is not to ignore review bots by default. The goal is to stop treating every comment as a required code change.
...
- Skip when: The comment body or a later reply from the same bot explicitly says the finding is withdrawn, compliant, or a false positive, and the agent can verify the relevant rule locally.
...
- Confidence: candidate
- Skip when: Practically never. When a diff replaces native browser behavior with a manual equivalent (native sticky → JS-positioned clones, native scroll targeting → forwarded wheel/touch events, paint-order occlusion → masks/clip-path), review-bot logic-bug findings against that code have been consistently legitimate.
