...
5. **Spawn one blinded judge** on a different model tier than the candidates' per the **arena** skill's Phase C. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from transcripts, not self-report.** Read each candidate's local transcript at `~/.claude/projects/<slug>/<session-id>/subagents/agent-<id>.jsonl`. Derive the path: `<slug>` is the absolute working directory with every character that is not a letter or digit replaced by `-`, and the current session id is `$CLAUDE_CODE_SESSION_ID`. Do not glob across `~/.claude/projects/*/`. That crosses workspace boundaries and reads private chats from unrelated projects. The JSONL is an internal format, so skip lines that fail to parse. Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
