...
4. Copy the skeleton below into the plan file and fill every placeholder. Unless the operator names a path, write the file under `~/.claude/projects/<slug>/docs/`, where `<slug>` is the absolute working directory with every character that is not a letter or digit replaced by `-`. Keep every heading and every sub-block in the order shown. One section per PR. One PR is one change with its own evidence (the **sequence-verifiable-units** principle skill). Name the execution playbook in **How to read this**. Pick between `playbooks/autopilot-full.md` and `playbooks/autopilot-stack.md` per the rule at the end of `playbooks/autopilot-stack.md`. A standing program takes `playbooks/orchestrate.md`.
...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use the `claude-in-chrome` skill or a Playwright MCP server (Electron via its CDP port). CLIs and TUIs use `Bash`, with a `tmux` session for TUIs. Claude Code's bundled `run` skill launches and drives the project's app. Native mobile uses whatever simulator-driving skill the repo has. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
- [ ] Arm the 30-minute audit tick. In a local session, `/loop 30m` with the tick prompt below. In a cloud root, a scheduled routine (`/schedule`) that fires the tick prompt in a cloud session. Never leave the cadence to memory.
...
Each live lane runs as its own subagent in its own git worktree (`isolation: "worktree"`, or `isolation: "remote"` when the session offers it) at the PR head. Drive through the `claude-in-chrome` skill or a Playwright MCP server for browser, Electron, and web UIs, `Bash` with a `tmux` session for CLIs and TUIs.
