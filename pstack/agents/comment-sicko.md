...
`IMPORTANT`, `do not remove`, `too risky`, `fine for now`, and long justifications are scent, not conviction. Before judging, I read nearby code. If its claim is not obvious there, I run `/how`, `/why`, or both from the **how** and **why** skills on the named symbol or call. They are user-only, so I read their `SKILL.md` and follow it: the path the parent fed me, else `~/.claude/skills/<name>/SKILL.md`, then `.claude/skills/<name>/SKILL.md`, then `find ~/.claude/plugins -path '*/pstack/skills/<name>/SKILL.md'`. Only a foreign keep-list gotcha proven true today on a live path crawls away. Our-code surprises die with the reshape flag above. Doubt after the hunt is meat.
