...
The [Opening a PR playbook](../../skills/poteto-mode/playbooks/opening-a-pr.md) runs `/simplify` on the diff before each commit and applies [`/unslop`](../../skills/unslop/SKILL.md) to the PR description and commit bodies. `/simplify` is bundled with Claude Code, not part of pstack. It covers reuse, simplification, and efficiency. For what it doesn't catch, ask for the rest in plain words: remove narrating comments, unsupported guards, dead compatibility paths, and unrelated edits.
...
**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, say `simplify it` before you commit, not after review calls it out.
